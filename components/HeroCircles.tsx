...
function SpeedSquareSvg({ style }: { style: CSSProperties }) {
  return (
    <svg
    ...
    >
      <path d="M0 0 L0 280 L280 280 Z" stroke="currentColor" strokeWidth="1" fill="none" />
      <path
...
export function HeroCircles() {
  return (
    <div className="absolute pointer-events-none inset-0 -z-10" aria-hidden="true">
      ...
      />
      {/* Tiny accent dot - right (match left 8px so composition stays centered) */}
      <div
        className="absolute rounded-full bg-border/40"
        style={{
          width: 8,
      ...
      />
      <SpeedSquareSvg style={{ left: -20 }} />
      <SpeedSquareSvg style={{ right: -20, left: "auto", transform: "scaleX(-1)" }} />
    </div>
  )
}
